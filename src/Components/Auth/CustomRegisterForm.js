...
const CustomRegisterForm = () => {
  ...
  const validate = (values) => {
    ...
    if (!errorRequired('password') && !isPaswordValid(values.password)) {
      errors.password =
        'La contraseña debe ser de al menos 6 caracteres, tambien debe incluir al menos una letra y un símbolo(@#$%)'
    }

    if (
      !errorRequired('repeatedPassword') &&
  ...
  const { touched, errors, values, handleChange, handleSubmit, handleBlur } =
    useFormik({
      initialValues: {
        email: '',
        password: '',
  ...
  return (
    <form className='form-container' onSubmit={handleSubmit}>
      ...
      <button id='registerButton' className='submit-btn' type='submit'>
        Registrarse
      </button>
    </form>
  )
